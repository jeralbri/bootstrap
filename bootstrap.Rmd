...
Start with the data described in Efron and Tibshriani (1993, page 19). We have 15 paired observations of student LSAT scores and GPS. 

We want to estimate the correlation between LSAT and GPAs. Summary statistics are the following:
...
2. Use the `boot` function to get `R` bootstrap replicates.
...
get_r <- function(x, y, data, indices) {
...
boot_out <- boot(x = "lsat", y = "gpa", R = 500, data = tbl, 
                 statistic = get_r)
...
3. What does it mean that the calculations and intervals are on the same scale?
...
In words: If the null hypothesis were true, we repeatedly draw samples of size $n$, and calculate $r$ each time, the probability we would observe an estimate of $|r| = `r r`$ or larger is less than 5%. 
...
We can see why by considering the 95% confidence interval for out estimate. The usual formula you see for a confidence interval is the estimate plus or minus the .975th quantile of the normal or $t$ distribution times the standard error.

$$
\text{95% CI} = r \pm t_{df = 13} SE_r
$$

If we were to sample 15 students repeatedly from our population and calculate this confidence interval each time, the interval would include the true population value 95% of the time.
...
4. Repeat $k$ times.
5. Use the distribution of estimates across the $k$ bootstrap samples as the sampling distribution.
...
2. The mean estimate of $r$ across the 500 bootstrap samples is `r boot_mean`. The difference between the mean of the bootstrap estimates and the original sample estimate is the _bias_.
3. The standard error is the standard deviation of the sampling distribution. Here the value is `r boot_se`, which is much smaller than our earlier estimate of `r r_se`. This is because `r r_se` was based on a formula that is only valid when $r = 0$.
...
This is smaller than our non-parametric bootstrap estimate of `r boot_se` reflective of the fact that our knowledge of the population distribution gives us more information that in turn reduces sampling variability. 
...
We saw that using the textbook standard error estimate for a correlation led us array because we ended up with an interval outside of the range of plausible values. 
...
Not all will be relevant for a bounded statistic like a correlation, though they may still be useful for unbounded statistics for which there is no clear formula for the standard error.
...
The only difference is that we use our bootstrap estimate of the standard error in the formula. In our example, we get

$$
\begin{align}
\text{95% CI} &= r \pm 1.96 \times SE_r \\
               &= `r r` \pm 1.96 \times `r boot_se` \\
               &= [`r round(r-1.96*boot_se,3)`, `r round(r+1.96*boot_se,3)`]
\end{align}
$$
...
where $\hat{\theta}$ is the sample estimate of the statistic, $\theta$ is the true population value (which we usually get from our null hypothesis), and $\widehat{SE}_{\theta}$ is the sampe estimate of the standard error. 
...
2. The bootstrap sample variance: $\frac{s^2}{n}$.
...
4. Save the bootstrap estimate of the statistic and the bootstrap-bootstrap estimate of the variance.
...
What does this look like in R? (Note: following code may need updating)
...
boot_t_out <- boot(x = "lsat", y = "gpa", its = 100, 
                   R = 100, data = tbl, statistic = get_r_var)
...
To do so, arrange the estimates in order from lowest to farthest, and use a quantile or percentile function to find the value at the 2.5 and 95.5 percentiles (given two-tailed $\alpha = .05$). 
...
where $\hat{\theta}_{(\cdot)}$ is the mean of the bootstrap estimates and $\hat{\theta}_{(i)})^2$ the estimate after deleting the $i$th case. The process of estimating a statistic $n$ times, each time dropping the $i \in N$ observations, is known as the _jackknife_ estimate.  
...
An equilvalent way of thinking about this is that it accounts for skew in the sampling distribution, like what we have seen in the prior histograms.
...
3. What does it mean that the calculations and intervals are on the same scale? - Not Answered
4. Why are some BCa intervals unstable? - Answered

---

What does it mean that calculations and intervals are on the same scale?
